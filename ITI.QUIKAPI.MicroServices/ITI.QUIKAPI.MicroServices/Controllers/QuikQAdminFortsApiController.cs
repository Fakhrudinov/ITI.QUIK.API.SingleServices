...
namespace ITI.QUIKAPI.MicroServices.Controllers
{
    ...
    public class QuikQAdminFortsApiController : ControllerBase
    {
        ...
        {
            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpGet CheckConnections/FortsApi Call");

            var result = _qService.CheckConnection();

            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpGet CheckConnections/FortsApi result OK={result.IsSuccess}");
        ...
        public IActionResult AddMatrixFortsCodeToTemplatePoKomissii([FromBody] TemplateAndFortsCodeModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPost AddMatrixFortsCode/ToTemplate/PoKomissii Error: {result.Messages[0]}");
                return Ok(result);
            }
        ...
        public IActionResult AddMatrixFortsCodeToTemplatePoPlechu([FromBody] TemplateAndFortsCodeModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPost AddMatrixFortsCode/ToTemplate/PoPlechu Error: {result.Messages[0]}");
                return Ok(result);
            }
        ...
        public IActionResult DeleteMatrixFortsCoedFromTemplatePoKomissii([FromBody] TemplateAndFortsCodeModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpDelete DeleteMatrixFortsCode/FromTemplate/PoKomissii Error: {result.Messages[0]}");
                return Ok(result);
            }
        ...
        public IActionResult DeleteMatrixFortsCoedFromTemplatePoPlechu([FromBody] TemplateAndFortsCodeModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpDelete DeleteMatrixFortsCode/FromTemplate/PoPlechu Error: {result.Messages[0]}");
                return Ok(result);
            }
        ...
        public IActionResult MoveMatrixFortsCodeBetweenTemplatesPoKomissii([FromBody] MoveMatrixFortsCodeModel moveModel)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPut Move/MatrixFortsCode/BetweenTemplates/PoKomissii Error: {result.Messages[0]}");
                return Ok(result);
            }

            result = _qService.MoveMatrixFortsCodeBetweenTemplates(true, moveModel);

            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPut Move/MatrixFortsCode/BetweenTemplates/PoKomissii result isOK={result.IsSuccess}");
        ...
        public IActionResult MoveMatrixFortsCodeBetweenTemplatesPoPlechu([FromBody] MoveMatrixFortsCodeModel moveModel)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPut Move/MatrixFortsCode/BetweenTemplates/PoPlechu Error: {result.Messages[0]}");
                return Ok(result);
            }

            result = _qService.MoveMatrixFortsCodeBetweenTemplates(false, moveModel);

            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} HttpPut Move/MatrixFortsCode/BetweenTemplates/PoPlechu result isOK={result.IsSuccess}");
        ...
        public IActionResult ReplaceAllMatrixFortsCodeInTemplatePoKomisii([FromBody] TemplateAndMatrixFortsCodesModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} Httppost ReplaceAll/MatrixFortsCode/InTemplate/PoKomisii Error: {result.Messages[0]}");
                return Ok(result);
            }

            result = _qService.ReplaceAllMatrixFortsCodesInTemplate(true, model);

            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} Httppost ReplaceAll/MatrixFortsCode/InTemplate/PoKomisii result isOK={result.IsSuccess}");
        ...
        public IActionResult ReplaceAllMatrixFortsCodeInTemplatePoPlechu([FromBody] TemplateAndMatrixFortsCodesModel model)
        {
            ...
            if (!result.IsSuccess)
            {
                _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} Httppost ReplaceAll/MatrixFortsCode/InTemplate/PoPlechu Error: {result.Messages[0]}");
                return Ok(result);
            }

            result = _qService.ReplaceAllMatrixFortsCodesInTemplate(false, model);

            _logger.LogInformation($"{DateTime.Now.ToString("HH:mm:ss:fffff")} Httppost ReplaceAll/MatrixFortsCode/InTemplate/PoPlechu result isOK={result.IsSuccess}");
